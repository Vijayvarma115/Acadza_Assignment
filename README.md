...
Here’s your content properly formatted as a clean **`README.md`** with clear structure, bold highlights, and polished formatting:

---

#  **How to Run — BKT + Ebbinghaus Version**

##  **Step 1 — Extract the ZIP**

Download `acadza-v2-bkt-ebbinghaus.zip` and extract it:

```bash
unzip acadza-v2-bkt-ebbinghaus.zip
cd acadza-v2
```
...
###  **Option B — Generate Outputs (No Server Needed)**
...
* Generates:
  ...
  * ✅ Recommendations
  * ✅ Leaderboard
* Output stored in: `sample_outputs/`

👉 **Fastest way to preview everything**

---

## 🌐 **Step 5 — Call API Endpoints**

### 🔍 Analyze Student (BKT + Forgetting Curve)

```bash
curl -X POST http://127.0.0.1:8000/analyze/STU_001
```

### 📋 Get DOST Plan

```bash
curl -X POST http://127.0.0.1:8000/recommend/STU_001
```

### ❓ Get Question Details

```bash
curl http://127.0.0.1:8000/question/Q_PHY_0018
```

### 🏆 Leaderboard (Exam Readiness Based)
...
## 🐞 **Step 6 — Test Debug Fix**

### ❌ Buggy Version (Same Recommendations for All)

```bash
python3 debug/recommender_buggy.py
```

### ✅ Fixed Version (Fully Personalized)

```bash
python3 debug/recommender_fixed.py
```
...
├── risk_scorer.py        ← Combines both → exam-day risk
├── adaptive_planner.py   ← Builds DOST plan from risk
├── data_loader.py        ← Data loading + normalization
├── generate_outputs.py   ← Batch output generator
├── requirements.txt
├── data/                 ← JSON data files (REQUIRED)
├── sample_outputs/       ← Pre-generated outputs
└── debug/
    ├── recommender_buggy.py
